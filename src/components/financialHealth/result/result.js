...
class YourResult extends React.Component {
  state = {
    ...
    checkDriveFolder: false,
    isProgressShow: false,
    isBtnShow: true
  };

  componentDidMount() {
    this.createFolders("checkDriveFolder", "folder created");
  ...
  createFolders = (route, message) => {
    ...
    fetch(baseurl + `/documentation/${route}/${this.props.userId}`, options)
      // fetch(
      //   `https://switchroo.herokuapp.com/financialHealth/${route}/${this.props.userId}`,
      //   options
      // )
      .then(res => {
      ...
      <div className="calculate-result">
        ...
        <div style={{ textAlign: "center", padding: "40px 0px" }}>
          {this.state.isBtnShow && (
            <button className="progresBtn" onClick={showProgress}>
...
const mapDispatchToProps = dispacth => ({
  ...
  onCalculateResult: (route, message) =>
    dispacth(Actions.calculateResults(route, message))
});
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(YourResult));
