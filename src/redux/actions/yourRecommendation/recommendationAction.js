...
const setText = userId => async dispatch => {
  try {
    ...
    // introduction
    let introduction = Object.keys(data.introduction).map(
      k => data.introduction[k]
    );
    payload.introduction = introduction;

    // circumstances
    let circumstances = Object.keys(data.circumstances).map(
      k => data.circumstances[k]
    );
    payload.circumstances = circumstances;

    // recommendation
    let recommendation = Object.keys(data.recommendation).map(
      k => data.recommendation[k]
    );
    payload.recommendation = recommendation;

    // applicants names
    payload.applicant1Name = data.applicant1Name;
    payload.applicant2Name = data.applicant2Name;
